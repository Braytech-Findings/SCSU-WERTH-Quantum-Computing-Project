...
## Publication Release Complete

The research question is answered in `FINAL_RESEARCH_ANSWER.md`. The definitive four-
figure PNG/PDF package is in `results/final_figures/publication/`, with plain-language
reading notes printed under every graph. It is regenerated by
`scripts/generate_publication_figures.R`, which validates the architecture row count,
emulator targets, circuit identities, repetitions, and 42,000 retrieved-shot total.

The answer is structural: the tested GHZ and QFT circuits require increasing routing
work on the line-connected superconducting proxy, while the all-to-all trapped-ion proxy
avoids routing SWAP insertion for those circuits. This is a controlled proxy conclusion,
not a physical provider ranking.
...
- Release date in citation metadata: `2026-07-23`
